...
import { ChevronDownIcon, ChevronRightIcon, TimerIcon } from "@radix-ui/react-icons";
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center gap-1.5 rounded-lg px-2 py-1.5 text-left hover:bg-zinc-100">
        ...
        <span className="body-small text-zinc-400">{zone.booths.length}</span>
      </CollapsibleTrigger>
            ...
            <li key={booth.boothId}>
              <button
                ...
                className={cn(
                  ...
                  selectedBoothId === booth.boothId && "bg-zinc-100",
                )}
              >
                <span className="flex min-w-0 items-center gap-1.5">
                  <TimerIcon className="size-3.5 shrink-0 text-zinc-400" />
...
export function BoothTreeSidebar({
  ...
  return (
    <div
      className={cn(
        "flex flex-col overflow-y-auto rounded-lg border border-zinc-200 bg-white p-3 shadow-md",
        className,
      )}
    >
      <p className="body-small-bold px-2 py-1.5 text-zinc-950">
        축제부스 {getTotalBoothCount(zones)}
      </p>
