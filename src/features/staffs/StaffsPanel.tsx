...
export function StaffsPanel({ festivalId }: { festivalId: string }) {
  /*
    스태프 로그인은 담당 축제 ID가 실린 초대 링크로만 된다. 아이디와 임시 비밀번호만
    받은 스태프가 그냥 /staff/login에 들어가면 「초대 링크로 접속해주세요」에 막히므로,
    관리자가 그대로 전달할 수 있는 전체 주소를 함께 보여준다.
  */
  const staffLoginUrl =
    typeof window === "undefined"
      ? staffLoginPath({ festivalId })
      : `${window.location.origin}${staffLoginPath({ festivalId })}`;
  ...
  */
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      createFieldStaff(festivalId, {
        loginId,
        name,
        // 백엔드가 앞뒤 공백을 잘라 저장하므로 빈 값은 아예 보내지 않는다.
        ...
        <section className="col-span-1 flex min-w-0 flex-col gap-4 rounded-lg border border-zinc-300 bg-white px-5 py-6 sm:px-8">
          ...
          {created ? (
            <TemporaryPasswordCard
              ...
              warning="임시 비밀번호는 지금만 확인할 수 있습니다. 스태프에게 아이디·접속 주소와 함께 바로 전달해주세요."
                ...
                <div key={staff.staffId} className="flex items-start gap-2 py-4">
                  ...
                  >
                    ...
                    <p className="body-small wrap-anywhere pl-4 text-zinc-500">
                      {staff.department ?? "-"} · {staff.loginId}
                    </p>
                  </Link>
                </div>
              ))}
